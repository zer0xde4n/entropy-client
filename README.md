...
Typescript client library for interacting with Entropy Markets. **This code, and Entropy Markets at large, draws inspiration and is friendly forked from Mango Markets.**
...
This example assumes that you have a wallet that is already setup with devnet tokens. The private key should be stored in `~/.config/solana/entropy-devnet-authority.json`. Visit https://v3.mango.markets/ and connect with the wallet to fund your margin account so that you can place orders. You can find the full source code in [example.ts](./src/example.ts).
